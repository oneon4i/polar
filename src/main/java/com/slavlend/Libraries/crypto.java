...
public class crypto {
    public PolarValue encrypt(PolarValue data, PolarValue secret) {
        ...
        try {
            ...
            PolarLogger.exception("Cryptography Error. It's a bug, send it to the developers! (NoSuchAlgo)", secret.instantiateAddress);
        } catch (NoSuchPaddingException e) {
            PolarLogger.exception("Cryptography Error. It's a bug, send it to the developers! (NoSuchPadding)", secret.instantiateAddress);
        } catch (InvalidKeyException e) {
            PolarLogger.exception("Cryptography Error. Invalid key! (Java): ", secret.instantiateAddress);
        } catch (IllegalBlockSizeException e) {
            PolarLogger.exception("Cryptography Error. Illegal Block Size! (Java): ", secret.instantiateAddress);
        } catch (BadPaddingException e) {
            PolarLogger.exception("Cryptography Error. Bad Padding! (Java): ", secret.instantiateAddress);
        }
    ...
    private SecretKey toKey(PolarValue bytesKey) {
        try {
            ...
            if (keyBytes.length > 32) {
                PolarLogger.exception("Cryptography Error. Key is too long!", bytesKey.instantiateAddress);
                return null;
            }
            ...
            PolarLogger.exception("Cryptography Error. Invalid key (Java): " + e.getCause().getMessage(), bytesKey.instantiateAddress);
        }
        return null;
    }
}
